...
class ConfigurableFakeLLM(BaseLLM):
    ...
    def _get_response_for_prompt(self, prompt: str, method_type: str) -> Any:
        # --- MODIFICATION START ---
        # Check responses_map using substring matching (longest match first)
        ...
        # Original if/elif chain for key lookup based on content
        # (This remains the fallback if responses_map doesn't match)
        ...
        if "JSON Output:" in prompt and "thoughts" in prompt:  # ToT Expansion
            ...
            key = "json_answer"
            # Use list cycling ONLY if not handled by responses_map
            # Note: This list cycling is still potentially problematic, but responses_map takes priority now.
        ...
        elif "Current Subquestion:" in prompt:  # LtM Solving (Text)
            key = "sub_answer"
            ...
            "Based on the following sequential subquestions"):  # Final answers (Text)
            key = "final_answer"
            ...
            if method_type == "generate":
                ...
                key, default = "generate_json", DEFAULT_JSON_RESPONSE  # Should be rarely reached now
            elif method_type == "_generate_json_internal_async":
                key, default = "generate_json_async", DEFAULT_ASYNC_JSON_RESPONSE  # Should be rarely reached now
        ...
        # Use default key lookup if no specific prompt content matched
        # (And not handled by responses_map or the specific JSON Answer list cycle)
    ...
    def _generate_json_internal(self, prompt: str, response_model: Type[BaseModel],
        ...
        if isinstance(response_obj, BaseModel):
            ...
            try:
                ...
                logger.warning(
                    f"Configured string response for JSON prompt is not valid JSON: {response_obj}")
    ...
    async def _generate_json_internal_async(self, prompt: str, response_model: Type[BaseModel],
        ...
        if isinstance(response_obj, BaseModel):
            ...
            try:
                ...
                logger.warning(
                    f"Configured string response for async JSON prompt is not valid JSON: {response_obj}")
                return "{}"
        try:
            return json.dumps(response_obj)
        except TypeError:
        ...
        if embs.shape[0] == 0 or n_clusters <= 0:
            ...
            if centers.ndim == 1 and output_dim > 0:
                ...
                centers = centers.reshape(n_clusters,
                                          1)  # Or shape (n_clusters,) depending on downstream use
