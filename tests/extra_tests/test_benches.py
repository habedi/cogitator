...
def mock_cot_methods(mocker):
    ...
    async def auto_run_async_side_effect(*args, **kwargs): await asyncio.sleep(
        0); return "AutoCoT Mock Async Output"

    async def cdw_run_async_side_effect(*args, **kwargs): await asyncio.sleep(
        0); return "CDWCoT Mock Async Output"

    async def sc_run_async_side_effect(*args, **kwargs): await asyncio.sleep(
        0); return "SC Mock Async Output"

    async def ltm_run_async_side_effect(*args, **kwargs): await asyncio.sleep(
        0); return "LtM Mock Async Output"

    async def tot_run_async_side_effect(*args, **kwargs): await asyncio.sleep(
        0); return "ToT Mock Async Output"

    async def got_run_async_side_effect(*args, **kwargs): await asyncio.sleep(
        0); return "GoT Mock Async Output"
...
# Define default values used in argparse definitions (copy from shared.py)
...
def mock_args_namespace_factory():
    """Factory to create mock argparse Namespace objects with correct defaults."""

    def _create(**kwargs):
        defaults = {
            'openai_key': None, 'debug': False,
            'dataset': DEFAULT_DATASET, 'cutoff': DEFAULT_CUTOFF,
            'provider': DEFAULT_PROVIDER, 'model_name': None,
...
def generation_parser():
    """Provides a real ArgumentParser instance for generation."""
...
def evaluation_parser():
    """Provides a real ArgumentParser instance for evaluation."""
    parser = argparse.ArgumentParser()
    benchmark_shared.add_common_args(parser)
...
# --- Tests for Benchmark Run/Evaluate Scripts (Keep these high-level) ---
...
async def test_benchmark_run_main_sync_flow(mocker, mock_load_dataset, mock_get_llm,
    ...
    config_file.touch()  # Use empty config, rely on CLI/defaults
    ...
    with patch.object(benchmark_shared.Datasets, "_process_gsm8k", return_value=(["q1"], ["g1"])):
        ...
        assert len(output_file.read_text().splitlines()) > 0  # Check if *something* was written
...
async def test_benchmark_evaluate_main_heuristic(mocker, tmp_path):
    ...
    await benchmark_evaluate.main()  # Just check it runs
...
strategies:
  ...
  CDWCoT: {enabled: false}
    ...
    assert merged_config['cutoff'] == DEFAULT_CUTOFF  # Check default


def test_load_config_cli_overrides_yaml(tmp_path, mock_args_namespace_factory, generation_parser):
    args = mock_args_namespace_factory(provider="ollama", use_async=False, dataset="csqa")
    yaml_content = """
generation:
  provider: openai
  use_async: true
  cutoff: 99
  dataset: aqua # YAML tries this dataset
    """
    config_file = tmp_path / "test_override.yml";
    config_file.write_text(yaml_content)
    merged_config = benchmark_shared.load_and_merge_config(args, generation_parser,
                                                           str(config_file), "generation")
    assert merged_config['provider'] == "ollama"  # CLI wins
    assert merged_config['use_async'] is False  # CLI wins
    assert merged_config['cutoff'] == 99  # YAML wins over default
    assert merged_config['dataset'] == "csqa"  # CLI wins
...
evaluation:
  ...
  extractor: {type: llm, provider: openai}
  show_details: true
    """
    ...
    assert merged_config['extractor_model_name'] is None
...
def test_load_config_malformed_yaml(tmp_path, mock_args_namespace_factory, generation_parser,
    ...
    with caplog.at_level(logging.WARNING):
        merged_config = benchmark_shared.load_and_merge_config(args, generation_parser,
                                                               str(config_file), "generation")
    assert "Error parsing malformed.yml" in caplog.text
    assert merged_config['provider'] == DEFAULT_PROVIDER
    assert merged_config['dataset'] == DEFAULT_DATASET
