...
def get_llm(provider: str, model_name: str, openai_key: Optional[str] = None) -> BaseLLM:
    ...
    if provider == "openai":
        ...
        key = openai_key or os.getenv("OPENROUTER_API_KEY")
        if not key:
            raise ValueError(
                "OpenRouter API key must be provided via --openai-key or "
                "OPENROUTER_API_KEY environment variable."
            )
        return OpenRouterLLM(api_key=key, model=model_name)
...
def parse_common_args(description: str) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model-name",
        default=None,
        help="Name of the model (default: 'gemma3:4b' for ollama, 'gpt-4.1' for openai)",
    )
    parser.add_argument(
        "--openai-key",
        default=None,
    ...
    if not args.model_name:
        if args.provider == "openai":
            args.model_name = "gpt-5.1-nano"
        elif args.provider == "openrouter":
            args.model_name = "openai/gpt-4o-mini"
        else:
            args.model_name = "gemma3:4b"
        logger.info(
