...
async def main_async(args: argparse.Namespace):
    ...
    logger.info("Running GraphOfThoughts asynchronously with trace...")
    tasks = [got.run_async(q, graph_of_operations=EXAMPLE_GOO, semaphore=semaphore, with_trace=True) for q in
             QUESTIONS]
        ...
        if trace:
            ...
            for node in trace.nodes:
                print(
                    ...
                    f"Score={node.score:.2f if node.score is not None else 'N/A'}, "
...
if __name__ == "__main__":
    run_main(main_sync, main_async, "Run Graph-of-Thoughts with trace example")
